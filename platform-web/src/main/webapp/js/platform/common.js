...
$.fn.extend({
    _openWindow: function (containerId, url, width, height, title ,extOptions) {
    ...
    _closeWindow: function (containerId) {
        $(this).close(containerId, this);
    },
    _close: function (containerId, window) {
        if(containerId && window){
            var $document = $(this)._getDocument(window);
            var $container = $document.find("#" + containerId);
            if($container.length > 0){
                try{
                    // 为了触发onClose事件
    ...
    _hadSelectOneRecord : function(gridId, callback){
        var grid = $("#" + gridId);
