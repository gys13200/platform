		...
		Ext.apply(Ext.util.Format, {
			...
			monetaryRenderer : function(val, metaData, model, row, col, store,
				...
				if (val) {
					...
					return Ext.String.format('<font style="color: {0};">{1}</font>',
                        (val > 0 ? 'blue' : 'red'),
                        Ext.util.Format.number(val, '0,000.00') + Ext.monetaryText);
			...
			dateRenderer : function(val, metaData, model, row, col, store, gridview) {
				...
				return Ext.String.format('<font style="color: #a40;">{0}</font>', Ext.util.Format.date(val, 'Y-m-d'));
			},

			// 浮点变量
			floatRenderer : function(val) {
